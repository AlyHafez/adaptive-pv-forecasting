...
def rolling_window_evaluation(
    ...
    for test_day in pd.date_range("2023-01-01", "2023-12-31", freq="D"):
        ...
        if len(window_predictions) == 0:
            ...
            continue
        if len(window_df) != len(window_predictions):
            logging.warning(f"Length mismatch for window ending {test_day.date()}: "
                f"df={len(window_df)}, preds={len(window_predictions)}, aligning")
        # align lengths
